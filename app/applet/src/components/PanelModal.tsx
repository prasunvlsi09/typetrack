...
export function PanelModal({ isOpen, onClose }: PanelModalProps) {
  ...
  const executeAction = async (action: 'disable' | 'enable' | 'admin' | 'unadmin' | 'dev' | 'undev', inputId: string) => {
    ...
    try {
      ...
      if (input.includes('@')) {
        ...
        // Fetch by ID to get the email
        const snapshot = await getDocs(query(collection(db, 'users'), where('uid', '==', input)));
        if (!snapshot.empty) {
          targetUserId = snapshot.docs[0].id;
          targetUserDoc = snapshot.docs[0].data();
        } else {
      ...
      const protectedEmails = ['eptoflprat@typetrack.local', 'rajarin@typetrack.local', 'pratyusalt@typetrack.local'];
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <motion.div
          ...
          className={`relative w-full max-w-4xl h-[80vh] overflow-hidden rounded-3xl shadow-2xl flex flex-col ${
                  ...
                  <div className={`p-4 rounded-xl border ${isLight ? 'bg-black/5 border-black/10' : 'bg-white/5 border-white/10'}`}>
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-2 w-full sm:w-auto">
                        ...
                        <button 
                          onClick={() => {
                            const input = (document.getElementById('message-input') as HTMLInputElement).value;
                            showMessage('success', `Message sent to everybody: ${input}`);
                          }}
                          className="px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg font-bold text-sm transition-colors whitespace-nowrap"
                        >
                          Execute
                        </button>
                      </div>

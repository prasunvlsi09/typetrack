...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      if (!currentUser) {
        ...
        const isAdminEmail = currentUser.email === 'rajarin@typetrack.local' || 
        ...
        if (currentUser.displayName) {
          const isOwnerOrAdmin = isAdminEmail;
          const isDev = isDevEmail && !isOwnerOrAdmin;
          
          let targetSuffix = '';
          if (isOwnerOrAdmin) targetSuffix = '.admin';
          else if (isDev) targetSuffix = '.dev';

          const currentName = currentUser.displayName;
          const baseName = currentName.replace(/\.admin$/i, '').replace(/\.dev$/i, '');
        ...
        if (isAdminEmail || isDevEmail) {
          ...
          if (currentUser.email === 'prasun.ece07@gmail.com' || currentUser.email === 'eptoflprat@typetrack.local') {
            role = 'owner';
  ...
  return (
    <AppContext.Provider value={{ view, setView, sessions, addSession, clearSessions, records, addRecord, notes, addNote, updateNote, deleteNote, settings, updateSettings, user, userRole, authLoading, sharedStream, setSharedStream, isTopeSidebarOpen, setIsTopeSidebarOpen, chats, setChats, currentChatId, setCurrentChatId, isWebSearchEnabled, setIsWebSearchEnabled, isDeepThinkingEnabled, setIsDeepThinkingEnabled, systemStatus, updateSystemStatus }}>
      {children}
    </AppContext.Provider>
  );
}

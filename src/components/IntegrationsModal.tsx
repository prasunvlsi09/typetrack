        ...
        >
          ...
          <div className="p-6 overflow-y-auto">
            ...
            <div className={`mt-6 p-4 rounded-xl border ${isLight ? 'bg-blue-50 border-blue-100 text-blue-800' : 'bg-blue-900/20 border-blue-800/50 text-blue-200'}`}>
              <p className="text-sm font-bold mb-2">GitHub Integration Setup:</p>
              <ul className="text-xs space-y-1 list-disc pl-4 opacity-80">
                <li>Ensure <strong>GITHUB_CLIENT_ID</strong> and <strong>GITHUB_CLIENT_SECRET</strong> are set in your environment secrets.</li>
                <li>In your GitHub App settings, set the <strong>Callback URL</strong> to:</li>
                <li className="font-mono break-all bg-black/5 p-1 rounded select-all">{window.location.origin}/auth/github/callback</li>
                <li>The <strong>Homepage URL</strong> should be:</li>
                <li className="font-mono break-all bg-black/5 p-1 rounded select-all">{window.location.origin}</li>
              </ul>
            </div>
            
            <div className={`mt-4 p-4 rounded-xl border ${isLight ? 'bg-amber-50 border-amber-100 text-amber-800' : 'bg-amber-900/20 border-amber-800/50 text-amber-200'}`}>
              <p className="text-sm">
                <strong>Note:</strong> Other integrations (Spotify, Slack, WhatsApp) are currently in prototype mode and do not require real API keys.
              </p>
            </div>
          </div>
        </motion.div>

...
export function TypingTest() {
  const { user, userRole, settings, updateSettings, setView, addSession, sessions } = useAppContext();
  const isLight = settings.theme === 'light';
  const isDev = user?.displayName?.endsWith('.dev') || 
                user?.displayName?.endsWith('.admin') || 
                userRole === 'admin' || 
                userRole === 'dev' || 
                userRole === 'owner';
        ...
        <div className="flex justify-center space-x-4">
          <MagneticEffect>
            <button
              ...
              className={`px-8 py-4 border rounded-full backdrop-blur-3xl backdrop-saturate-200 transition-all duration-300 font-semibold flex items-center space-x-2 ${isLight ? 'bg-white/60 hover:bg-white/80 border-white/50 shadow-lg text-black' : 'bg-white/5 hover:bg-white/10 border-white/10 shadow-lg text-white'}`}
            >
          ...
          {!sessionSaved && (
            <MagneticEffect>
              <button
                ...
                className="px-8 py-4 bg-emerald-500/20 hover:bg-emerald-500/30 border border-emerald-500/50 rounded-full backdrop-blur-3xl backdrop-saturate-200 shadow-lg transition-all duration-300 text-emerald-500 font-semibold flex items-center space-x-2"
              >

...
export function TypingTest() {
  ...
  const isDev = user?.displayName?.endsWith('.dev') || 
                ...
                user?.displayName?.endsWith('.owner') || 
                userRole === 'admin' || 
                userRole === 'dev' || 
                userRole === 'owner';
        ...
        <div className="flex justify-center space-x-4">
          <MagneticEffect>
            <button
              ...
              className={`px-8 py-4 rounded-full transition-all duration-300 font-semibold flex items-center space-x-2 ${isLight ? 'apple-glass-light text-black hover:bg-white/60' : 'apple-glass-dark text-white hover:bg-white/10'}`}
            >

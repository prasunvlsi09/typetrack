  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'signup' || mode === 'devsignup') {
        ...
        const isOwnerEmail = email === 'rajarin@typetrack.local' || email === 'pratyus@typetrack.local' || email === 'pratyusalt@typetrack.local' || email === 'prasun.ece07@gmail.com' || email === 'eptoflprat@typetrack.local';
        const suffix = isOwnerEmail ? '.owner' : '.dev';
        
        // Strip any manually added .owner, .admin or .dev to prevent spoofing
        const cleanDisplayName = displayName.trim().replace(/\.owner$/i, '').replace(/\.admin$/i, '').replace(/\.dev$/i, '');
    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          >
            <div className={`${mode === 'devlog' || mode === 'devsignup' || mode === 'dev_select' ? 'apple-glass-light text-black' : 'apple-glass-dark text-white'} rounded-3xl overflow-y-auto max-h-[85vh] relative`}>
              {/* Close Button */}
              <button
                onClick={onClose}
                      ...
                      {mode === 'devsignup' ? (
                        <div className="flex items-center w-full px-4 py-3 border rounded-xl focus-within:ring-1 transition-all bg-black/5 border-black/10 text-black focus-within:border-black/50 focus-within:ring-black/50">
                          ...
                          <span className="text-black/50 font-medium select-none">
                            {username && ['rajarin', 'pratyus', 'pratyusalt', 'eptoflprat'].includes(username.toLowerCase().replace(/[^a-z0-9]/g, '')) ? '.owner' : '.dev'}
                          </span>
                        </div>
                      ) : (
                        <input

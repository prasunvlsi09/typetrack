    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            <div className={`${mode === 'devlog' || mode === 'devsignup' || mode === 'dev_select' ? 'bg-white/60 text-black border-white/50' : 'bg-black/60 text-white border-white/10'} backdrop-blur-3xl backdrop-saturate-200 border rounded-3xl shadow-2xl overflow-y-auto max-h-[85vh] relative`}>
              {/* Close Button */}
              <button
                onClick={onClose}

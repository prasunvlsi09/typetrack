...
export function PanelModal({ isOpen, onClose }: PanelModalProps) {
  ...
  const executeAction = async (action: 'disable' | 'enable' | 'admin' | 'unadmin' | 'dev' | 'undev' | 'delete' | 'history', inputId: string) => {
                ...
                <div className="space-y-4">
                  ...
                  <div className={`p-4 rounded-xl border ${isLight ? 'bg-black/5 border-black/10' : 'bg-white/5 border-white/10'}`}>
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      <div>
                        <div className="font-mono font-bold text-teal-500 text-lg">/history</div>
                        <div className={`text-sm mt-1 ${isLight ? 'text-black/70' : 'text-white/70'}`}>
                          Pulls up the history for an account (sessions, records, stats).
                        </div>
                      </div>
                      <div className="flex items-center gap-2 w-full sm:w-auto">
                        <input 
                          ...
                          className={`px-3 py-2 rounded-lg text-sm flex-1 sm:w-48 outline-none border focus:border-teal-500 transition-colors ${
                            isLight ? 'bg-white border-black/20 text-black' : 'bg-black/50 border-white/20 text-white'
                          }`}
                        />
                        <button 
                          onClick={() => executeAction('history', 'history-input')}
                          disabled={actionLoading}
                          className="px-4 py-2 bg-teal-500 hover:bg-teal-600 disabled:opacity-50 text-white rounded-lg font-bold text-sm transition-colors whitespace-nowrap"
                        >
                          Execute
                        </button>
                      </div>

...
const SCRIPTS = [
  ...
  { cmd: '/owner', action: 'owner' as const, color: 'text-yellow-500', bg: 'bg-yellow-500', hover: 'hover:bg-yellow-600', desc: 'Make someone an owner (adds .owner suffix and gives owner powers).', placeholder: 'User Email or ID' },
  { cmd: '/unowner', action: 'unowner' as const, color: 'text-orange-500', bg: 'bg-orange-500', hover: 'hover:bg-orange-600', desc: 'Remove someone\'s owner powers.', placeholder: 'User Email or ID' },
...
export function PanelModal({ isOpen, onClose }: PanelModalProps) {
  ...
  const executeAction = async (action: 'disable' | 'enable' | 'admin' | 'unadmin' | 'owner' | 'unowner' | 'dev' | 'undev' | 'delete' | 'history' | 'message' | 'maintenance' | 'unmaintenance', input: string, clearCallback?: () => void) => {
    input = input?.trim();
    ...
    try {
      ...
      for (const target of targetUsers) {
        ...
        switch (action) {
          ...
          case 'owner':
            updates = { role: 'owner' };
            if (targetUserDoc?.displayName && !targetUserDoc.displayName.endsWith('.owner')) {
              updates.displayName = `${targetUserDoc.displayName.replace(/\.admin$/, '').replace(/\.dev$/, '')}.owner`;
            }
            break;
          case 'unowner':
            updates = { role: 'casual' };
            if (targetUserDoc?.displayName && targetUserDoc.displayName.endsWith('.owner')) {
              updates.displayName = targetUserDoc.displayName.replace(/\.owner$/, '');
            }
            break;
          case 'admin':
            updates = { role: 'admin' };
            if (targetUserDoc?.displayName && !targetUserDoc.displayName.endsWith('.admin')) {
              updates.displayName = `${targetUserDoc.displayName.replace(/\.owner$/, '').replace(/\.dev$/, '')}.admin`;
            }
            break;
          case 'unadmin':
            updates = { role: 'casual' };
          ...
          case 'dev':
            ...
            if (targetUserDoc?.displayName && !targetUserDoc.displayName.endsWith('.dev')) {
              updates.displayName = `${targetUserDoc.displayName.replace(/\.owner$/, '').replace(/\.admin$/, '')}.dev`;
            }
            break;
          case 'undev':
            updates = { role: 'casual' };
      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            className={`relative w-full max-w-4xl h-[80vh] overflow-hidden rounded-3xl flex flex-col ${

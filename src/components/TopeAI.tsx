...
import { Send, User, Loader2, Plus, Search, Pin, Trash2, Edit2, ChevronLeft, ChevronRight, MessageSquare, Monitor, MonitorOff, ChevronDown, ChevronUp, Globe, Brain } from 'lucide-react';
...
export function TopeAI() {
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: -20 }}
      className={`max-w-6xl mx-auto h-[calc(100vh-12rem)] flex border rounded-3xl overflow-hidden backdrop-blur-3xl backdrop-saturate-200 shadow-2xl relative ${isLight ? 'bg-white/60 border-white/50' : 'bg-black/60 border-white/10'}`}
    >
      {/* Sidebar */}
      <AnimatePresence initial={false}>
        {isSidebarOpen && (
          <motion.div
            ...
            className={`flex-shrink-0 border-r flex flex-col h-full overflow-hidden ${isLight ? 'bg-white/40 border-black/10' : 'bg-black/40 border-white/10'}`}
          >
              ...
              {isLoading && (
                <div className="flex justify-start">
                  <div className="flex max-w-[80%] space-x-3">
                    ...
                    <div className={`p-4 rounded-2xl rounded-tl-sm flex items-center space-x-2 ${isLight ? 'bg-black/5 text-black' : 'bg-white/10 text-white'}`}>
                      <Loader2 className={`w-4 h-4 animate-spin ${isLight ? 'text-black/50' : 'text-white/50'}`} />
                      <span className={`text-sm ${isLight ? 'text-black/50' : 'text-white/50'}`}>Tope is thinking...</span>
                    </div>
                  </div>
                </div>
              )}

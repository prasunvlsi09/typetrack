...
export function AccLogsModal({ isOpen, onClose }: AccLogsModalProps) {
  ...
  useEffect(() => {
    const isAdmin = 
      ...
      (user?.displayName?.endsWith('.owner') && user?.email?.endsWith('@typetrack.local')) || 
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        ...
        >
          <motion.div
            key="acc-logs-content"
            initial={{ scale: 0.95, y: 20 }}
            animate={{ scale: 1, y: 0 }}
            exit={{ scale: 0.95, y: 20 }}
            onClick={(e) => e.stopPropagation()}
            className={`relative w-full max-w-4xl max-h-[80vh] overflow-hidden rounded-3xl flex flex-col ${
                ...
                {users.map((u) => (
                  ...
                  >
                    <div className="flex items-center space-x-4">
                      {u.photoURL ? (
                        <img src={u.photoURL} alt="Profile" className="w-12 h-12 rounded-full object-cover" referrerPolicy="no-referrer" />
                      ) : (
                      ...
                      <div>
                        <h3 className="font-bold text-lg flex items-center gap-2">
                          ...
                          {((u.displayName?.endsWith('.owner') && u.email?.endsWith('@typetrack.local')) || u.email === 'rajarin@typetrack.local' || u.email === 'pratyus@typetrack.local' || u.email === 'pratyusalt@typetrack.local' || u.email === 'prasun.ece07@gmail.com' || u.email === 'eptoflprat@typetrack.local') ? (
                            <span className="text-xs bg-yellow-500 text-white px-2 py-0.5 rounded-full">Owner</span>
                          ) : (u.displayName?.endsWith('.admin') && u.email?.endsWith('@typetrack.local')) ? (

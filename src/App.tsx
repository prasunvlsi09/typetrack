...
import { Keyboard, LayoutDashboard, Trophy, Home as HomeIcon, LogIn, LogOut, Moon, Sun, Bot, Users, Mail, FileText, Shield, X } from 'lucide-react';
    ...
    const unsub = onSnapshot(doc(db, 'system', 'broadcast'), (docSnap) => {
      if (docSnap.exists()) {
        ...
        if (data.timestamp && Date.now() - data.timestamp < 60000 && data.message) {
          setMessage({ text: data.message, id: data.timestamp });
        }
      }
    });
    return () => unsub();
        ...
        >
          <div className="bg-blue-600 text-white p-4 rounded-2xl shadow-2xl border border-blue-400/30 flex items-start gap-3">
            <div className="flex-1">
              <h4 className="font-bold text-blue-100 text-sm uppercase tracking-wider mb-1">System Broadcast</h4>
      ...
      <header className="fixed top-0 left-0 right-0 z-50 p-6 flex justify-between items-center">
        ...
        {!authLoading && (
          <div className="flex items-center space-x-4">
            {user ? (
              <div className={`flex items-center space-x-4 ${isLight ? 'bg-black/5 border-black/10' : 'bg-white/10 border-white/20'} backdrop-blur-md border rounded-full pl-2 pr-4 py-1.5`}>
                {isAdmin && (
                  <button
...
function Navigation() {
  ...
  return (
    <nav className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50">
      <div className={`flex items-center space-x-2 ${isLight ? 'bg-black/5 border-black/10' : 'bg-white/10 border-white/20'} backdrop-blur-xl border p-2 rounded-full shadow-[0_8px_32px_0_rgba(31,38,135,0.37)]`}>
        {navItems.map((item) => {

      ...
      <header className="fixed top-0 left-0 right-0 z-50 p-6 flex justify-between items-center">
        ...
        {!authLoading && (
          <div className="flex items-center space-x-4">
            {user ? (
              <div className={`flex items-center space-x-4 ${isLight ? 'bg-white/40 border-white/50 shadow-lg' : 'bg-black/40 border-white/10 shadow-lg'} backdrop-blur-2xl backdrop-saturate-200 border rounded-full pl-2 pr-4 py-1.5`}>
                {isAdmin && (
                  <MagneticEffect>
                    <button
...
function Navigation() {
  ...
  return (
    <nav className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50">
      <div className={`flex items-center space-x-2 ${isLight ? 'bg-white/40 border-white/50 shadow-lg' : 'bg-black/40 border-white/10 shadow-lg'} backdrop-blur-2xl backdrop-saturate-200 border p-2 rounded-full`}>
        {navItems.map((item) => {

...
import { Keyboard, LayoutDashboard, Trophy, Home as HomeIcon, LogIn, LogOut, Moon, Sun, Bot, Users, Mail, FileText, Shield, X, BookOpen, ChevronDown, ChevronUp, Wrench } from 'lucide-react';
...
function Header() {
  const { user, userRole, authLoading, settings, updateSettings, systemStatus, photoURL } = useAppContext();
  ...
  const isDev = userRole === 'dev' || userRole === 'admin' || userRole === 'owner' || user?.displayName?.endsWith('.dev') || user?.displayName?.endsWith('.admin') || user?.displayName?.endsWith('.owner');
  const isAdmin = userRole === 'admin' || userRole === 'owner' || (user?.displayName?.endsWith('.admin') && user?.email?.endsWith('@typetrack.local')) || (user?.displayName?.endsWith('.owner') && user?.email?.endsWith('@typetrack.local'));
  const hasPanelAccess = userRole === 'owner' || ['eptoflprat@typetrack.local', 'rajarin@typetrack.local', 'pratyusalt@typetrack.local', 'pratyus@typetrack.local', 'prasun.ece07@gmail.com'].includes(user?.email || '');
      ...
      <header className="fixed top-0 left-0 right-0 z-50 p-6 flex justify-between items-center">
        ...
        {!authLoading && (
          <div className="flex items-center space-x-4">
            {user ? (
              <div className={`flex items-center space-x-4 ${isLight ? 'apple-glass-light' : 'apple-glass-dark'} rounded-full pl-2 pr-4 py-1.5`}>
                {isAdmin && (
                  <MagneticEffect>
                    <button
                ...
                <MagneticEffect>
                  ...
                  >
                    {photoURL ? (
                      <img src={photoURL} alt="Profile" className="w-8 h-8 rounded-full object-cover" referrerPolicy="no-referrer" />
                    ) : (
...
function Navigation() {
  const { view, setView, settings, user, userRole } = useAppContext();
      ...
      <AnimatePresence>
        {isVisible && (
          <motion.nav 
            ...
            className="mb-3"
          >
            <div className={`flex items-center space-x-2 ${isLight ? 'apple-glass-light' : 'apple-glass-dark'} p-2 rounded-full`}>
              {navItems.map((item) => {
...
function AppLayout() {
  const { settings, isTopeSidebarOpen, setIsTopeSidebarOpen, user, isMaintenance } = useAppContext();
  const isLight = settings.theme === 'light';

  if (isMaintenance) {
    return (
      <div className={`min-h-screen flex flex-col items-center justify-center p-6 ${isLight ? 'bg-white text-black' : 'bg-[#050505] text-white'} font-sans`}>
        <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none">
          <div className={`absolute top-[-10%] left-[-10%] w-[50%] h-[50%] rounded-full ${isLight ? 'bg-orange-300/40' : 'bg-orange-600/30'} blur-[120px] mix-blend-screen animate-blob`} />
          <div className={`absolute bottom-[-20%] right-[-10%] w-[60%] h-[60%] rounded-full ${isLight ? 'bg-red-300/30' : 'bg-red-600/20'} blur-[120px] mix-blend-screen animate-blob animation-delay-2000`} />
        </div>
        <div className="relative z-10 flex flex-col items-center text-center max-w-md">
          <div className="w-24 h-24 bg-orange-500/20 rounded-full flex items-center justify-center mb-8">
            <Wrench className="w-12 h-12 text-orange-500" />
          </div>
          <h1 className="text-4xl font-bold mb-4 tracking-tight">Under Maintenance</h1>
          <p className={`text-lg mb-8 ${isLight ? 'text-black/70' : 'text-white/70'}`}>
            Your account is currently under maintenance. Please check back later.
          </p>
          <button
            onClick={logOut}
            className="px-8 py-3 bg-emerald-500 text-white font-bold rounded-full hover:bg-emerald-400 transition-colors"
          >
            Log Out
          </button>
        </div>
  ...
  return (
    <div className={`min-h-screen ${isLight ? 'bg-white text-black selection:bg-emerald-400/30 selection:text-emerald-800' : 'bg-[#050505] text-white selection:bg-emerald-400/30 selection:text-emerald-200'} font-sans overflow-x-hidden transition-colors duration-300`}>
      {/* Liquid Glass Background Elements */}
      <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none">
        <div className={`absolute top-[-10%] left-[-10%] w-[50%] h-[50%] rounded-full ${isLight ? 'bg-purple-300/40' : 'bg-purple-600/30'} blur-[120px] mix-blend-screen animate-blob`} />
        <div className={`absolute top-[20%] right-[-10%] w-[60%] h-[60%] rounded-full ${isLight ? 'bg-blue-300/30' : 'bg-blue-600/20'} blur-[120px] mix-blend-screen animate-blob animation-delay-2000`} />
        <div className={`absolute bottom-[-20%] left-[20%] w-[70%] h-[70%] rounded-full ${isLight ? 'bg-emerald-300/30' : 'bg-emerald-600/20'} blur-[120px] mix-blend-screen animate-blob animation-delay-4000`} />
        <div className={`absolute top-[40%] left-[40%] w-[40%] h-[40%] rounded-full ${isLight ? 'bg-pink-300/20' : 'bg-pink-600/10'} blur-[120px] mix-blend-screen animate-blob animation-delay-2000`} />
      </div>

...
function Header() {
  const { user, authLoading, settings, updateSettings, systemStatus } = useAppContext();
  ...
  const isDev = user?.displayName?.endsWith('.dev') || user?.displayName?.endsWith('.admin') || user?.email === 'prasun.ece07@gmail.com';
  const isAdmin = 

...
function Header() {
  ...
  const isDev = userRole === 'dev' || userRole === 'admin' || userRole === 'owner' || user?.displayName?.endsWith('.dev') || user?.displayName?.endsWith('.admin');
  const isAdmin = userRole === 'admin' || userRole === 'owner' || (user?.displayName?.endsWith('.admin') && user?.email?.endsWith('@typetrack.local'));
